...
class handle {
  public:
    ...
    virtual ~handle() {
        if (m_destructor) { m_destructor(m_resource); }
    }

    // Copying not allowed
    handle(handle<T> const &other)                        = delete;
    auto operator=(handle<T> const &other) -> handle<T> & = delete;
...
class gluint_handle : public handle<T> {
  public:
    ...
    gluint_handle(destructor &&_destructor, C &&_constructor, Args &&...args)
        ...
        if (this->value().id() == 0) { throw std::runtime_error("Invalid gluint_handle"); }
    }
...
class vertex_array_handle : public gen_delete_handle<vertex_array_id> {
  public:
    vertex_array_handle()
        : gen_delete_handle<vertex_array_id>(glDeleteVertexArrays, glGenVertexArrays) {}
};
...
class framebuffer_handle : public gen_delete_handle<framebuffer_id> {
  public:
    framebuffer_handle()
        : gen_delete_handle<framebuffer_id>(glDeleteFramebuffers, glGenFramebuffers) {}
};
