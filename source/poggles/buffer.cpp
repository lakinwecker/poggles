...
poggles::buffer::buffer(GLenum target, GLuint index, GLint size, GLenum data_type)
    : m_target(target) {
    gl::bindBuffer(target, static_cast<buffer_id>(m_buffer_handle));
    if (data_type == GL_DOUBLE) {
        glVertexAttribLPointer(index, size, data_type, 0, static_cast<void *>(nullptr));
    } else {
        glVertexAttribPointer(index, size, data_type, GL_FALSE, 0, static_cast<void *>(nullptr));
    }
    glEnableVertexAttribArray(index);
}

poggles::buffer::buffer(
    GLenum target,
    GLuint start_index,
    GLuint num_indices,
